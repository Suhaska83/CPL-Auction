...
export function AuctionLiveView({ initial }: { initial: LiveSnapshot }) {
  const [snap, setSnap] = useState<LiveSnapshot>(initial);

  useEffect(() => {
    const socket: Socket = io({ path: "/socket.io" });
    socket.on("state", (payload: LiveSnapshot) => setSnap(payload));
    // Polling fallback: refresh every 5s in case websockets are blocked
  ...
  const { currentPlayer, currentTeam, state, counts } = snap;
  ...
  const bidAmount = state.currentBid > 0 ? state.currentBid : currentPlayer.basePrice;
  ...
  return (
    <div className="flex items-center overflow-hidden rounded-md ring-1 ring-panel-border">
      ...
      <span className="bg-brand-red px-3 py-1 font-display text-sm font-bold text-white">
        {formatCompact(value) === `${value}` ? value : value}
      </span>
    </div>
  );
}
